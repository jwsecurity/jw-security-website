...
const CaseStudiesPage = () => {
  ...
  return (
    <>
      <Header />
      <PageHero title="Case Studies" subtitle="Before/after results from recent residential and commercial work" backgroundImage="/images/jw/locksmith-maintenance.jpg" minHeight="40vh" centerContent />
      <Box sx={{ py: { xs: 6, md: 8 } }}>
        <Container>
            ...
            {studies.map((s, i) => (
              <Grid item xs={12} sm={6} md={3} key={i}>
                <Card sx={{ height: '100%', borderRadius: 2, border: `1px solid ${alpha(JW_BLUE, 0.08)}`, boxShadow: '0 6px 16px rgba(0,0,0,0.06)' }}>
                  <CardContent>
                    <Typography variant="overline" sx={{ color: JW_CYAN }}>{s.type}</Typography>
                    ...
                    <Box sx={{ display: 'flex', gap: 1.5, mb: 2 }}>
                      <Box sx={{ flex: 1 }}>
                        ...
                        <Typography variant="caption" sx={{ display: 'block', mt: 0.5, color: 'text.secondary' }}>Before</Typography>
                      </Box>
                      <Box sx={{ flex: 1 }}>
                        <Placeholder aria-label="After photo placeholder" />
                        <Typography variant="caption" sx={{ display: 'block', mt: 0.5, color: 'text.secondary' }}>After</Typography>
                      </Box>
                    </Box>
                    <Button size="small" disabled sx={{ textTransform: 'none' }}>Full gallery coming soon</Button>
                  </CardContent>
                </Card>
              </Grid>
            ))}
