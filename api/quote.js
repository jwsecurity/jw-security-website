...
export default async function handler(req, res) {
  ...
  const formattedProperty = PROPERTY_LABELS[propertyType] || escapeHtml(propertyType) || 'Not specified';
  const formattedUrgency = URGENCY_LABELS[urgency] || escapeHtml(urgency) || 'Not specified';
  const formattedDate = escapeHtml(formatPreferredDate(preferredDate));

  try {
    const jwSecurityEmail = await resend.emails.send({
    ...
    res.status(200).json({
      ...
      ids: {
        jwSecurity: jwSecurityEmail.data?.id,
